...
pv_location["cluster"] = kmeans.fit_predict(pv_location[["coord1", "coord2"]]).astype(
    int
)
...
cluster_means = df.groupby(["hour", "cluster"])[select_column].mean().reset_index()
...
print(
    f"Added per cluster: {[f'cluster_{i}_nins / cluster_{i}_ratio' for i in range(num_cluster)]}"
)
